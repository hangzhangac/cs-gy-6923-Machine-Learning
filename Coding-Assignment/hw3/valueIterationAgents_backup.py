    ...
    def runValueIteration(self):
        ...
        for state in all_states:
           ...
           for action in actions:
                next_state_probs=mdp.getTransitionStatesAndProbs(state,action)
                for next_state,prob in next_state_probs:
                    if next_state not in preds:
                        preds[next_state]=set()
        ...
        for state in all_states:
            ...
            for action in actions:
                ...
                for next_state,prob in next_state_probs:
                    reward=mdp.getReward(state,action,next_state)
                    expected_sum+=prob*(reward+discount*self.values[next_state])
                max_v=max(max_v, expected_sum)
            diff=abs(max_v-cur_value)
            pq.push(state,-diff)
        ...
        for k in range(self.iterations):
            if pq.isEmpty():
                break
            cur_state=pq.pop()
            #self.values[cur_state]=value
            if not mdp.isTerminal(cur_state):
                actions=mdp.getPossibleActions(cur_state)
                max_v=-inf
                for action in actions:
                    next_state_probs=mdp.getTransitionStatesAndProbs(cur_state,action) 
                    expected_sum=0
                    for next_state,prob in next_state_probs:
                        reward=mdp.getReward(cur_state,action,next_state) 
                        expected_sum+=prob*(reward+discount*self.values[next_state])
                    max_v=max(max_v,expected_sum)
                self.values[cur_state]=max_v
            all_preds=preds[cur_state]
            for state in all_preds:
                if mdp.isTerminal(state):
                    continue
                cur_value=self.values[state]
                actions=mdp.getPossibleActions(state)
